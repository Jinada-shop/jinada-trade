...
LANG = {
    "ru": {
        ...
        "sign_in": "Войти", "free_trial": "Пробный доступ (3 дня)",
        ...
        "days_left": "дн.", "bot_status": "Бот активен 24/7",
        ...
        "keys_ok": "Ключи сохранены!", "keys_removed": "Ключи удалены", "keys_saved": "Ключи настроены",
    ...
    "en": {
        ...
        "days_left": "d left", "bot_status": "Bot Active 24/7",
        ...
        "keys_ok": "Keys saved!", "keys_removed": "Keys removed", "keys_saved": "Keys configured",
...
def check_password(password, h, salt):
    return hashlib.sha256(f"{password}{salt}".encode()).hexdigest() == h

def create_client(username, password, plan="trial"):
    ...
    pi = plans.get(plan, plans["trial"])
    pw, salt = hash_password(password)
    clients["clients"][username] = {
        "username": username, "password_hash": pw, "salt": salt,
        "plan": plan, "plan_name": pi["name"], "days": pi["days"],
        "expires": (datetime.now() + timedelta(days=pi["days"])).isoformat(),
...
def verify_client(username, password):
    ...
    if username not in clients["clients"]:
        return None
    c = clients["clients"][username]
    if not c.get("active"): return None
    if not check_password(password, c["password_hash"], c["salt"]): return None
    if datetime.now() > datetime.fromisoformat(c["expires"]):
        c["active"] = False; save_clients(clients); return None
    return c
...
def main():
    ...
    st.markdown("""<style>
        ...
        * { font-family: 'Inter', sans-serif; } .stApp { background: #0A0A0A; }
        h1, h2, h3 { color: #FFF !important; font-weight: 700 !important; } h1 { color: #FFD700 !important; }
        .stButton > button { background: #FFF; color: #000; border: none; border-radius: 10px; padding: 14px 28px; font-weight: 700; font-size: 15px; width: 100%; cursor: pointer; }
        .stButton > button:hover { background: #E5E5E5; }
        .stTextInput > div > div > input, .stSelectbox > div > div { background: #141414; border: 1px solid #222; border-radius: 10px; color: #FFF; padding: 14px 16px; }
        [data-testid="stMetricValue"] { color: #FFF !important; font-weight: 800 !important; }
        .card { background: #111; border: 1px solid #1A1A1A; border-radius: 14px; padding: 24px; margin-bottom: 12px; }
        .badge { display: inline-block; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
        .badge.green { background: rgba(0,255,136,0.12); color: #00FF88; } .badge.gold { background: rgba(255,215,0,0.12); color: #FFD700; } .badge.red { background: rgba(255,68,68,0.12); color: #FF4444; }
        ...
        .stTabs [aria-selected="true"] { color: #FFD700 !important; } hr { border-color: #1A1A1A !important; margin: 24px 0 !important; }
        .green-text { color: #00FF88; } .red-text { color: #FF4444; }
    ...
    if not st.session_state.logged_in:
        ...
        with col2:
            ...
            st.markdown(f"""<div style="text-align:center;padding:20px 0;"><h1 style="font-size:36px;margin:0;">{t['title']}</h1><p style="color:#666;font-size:15px;">{t['subtitle']}</p></div>""", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with tab1:
                ...
                if st.button(t['sign_in'], type="primary"):
                    if not u or not p: st.error(t['fill_fields'])
                    elif u == ADMIN_USERNAME and p == ADMIN_PASSWORD:
                        st.session_state.logged_in = True; st.session_state.username = ADMIN_USERNAME
                        st.session_state.is_admin = True
                        st.session_state.client = {"username":ADMIN_USERNAME,"plan":"lifetime","plan_name":"Admin","active":True,"expires":(datetime.now()+timedelta(days=99999)).isoformat(),"api_key":"","api_secret":"","exchange":"","balance":0}
                        st.rerun()
                    else:
                        c = verify_client(u, p)
                        if c: st.session_state.logged_in = True; st.session_state.username = u; st.session_state.client = c; st.session_state.is_admin = False; st.rerun()
                        else: st.error(t['wrong_creds'])
            with tab2:
                nu = st.text_input(t['username'], key="reg_user")
                np = st.text_input(t['password'], type="password", key="reg_pass")
                if st.button(t['free_trial'], type="primary"):
                    if len(nu)<3: st.error(t['username_short'])
                    elif len(np)<4: st.error(t['password_short'])
                    elif create_client(nu, np, "trial"): st.success(t['created'])
    ...
    with col1:
        ...
        ab = '<span class="badge gold" style="margin-left:8px;">ADMIN</span>' if is_admin else ''
        st.markdown(f"""<div style="display:flex;align-items:center;gap:12px;"><h2 style="margin:0;font-size:24px;">{t['title']}</h2><span class="badge {color}">{client['plan_name']}</span>{ab}<span style="color:#555;font-size:13px;">{days_left}{t['days_left']}</span></div>""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.caption(client.get('exchange', '-'))
    ...
    with c3:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.metric(t['pnl_today'], "$0.00", delta="0%")
        st.markdown('</div>', unsafe_allow_html=True)
    
    # График
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown(f"#### 📈 {t['balance_chart']}")
    dates = pd.date_range(datetime.now() - timedelta(days=7), periods=7, freq='D')
    balances = [300, 302, 305, 301, 308, 312, 315]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dates, y=balances, mode='lines', line=dict(color='#FFD700', width=2), fill='tozeroy', fillcolor='rgba(255,215,0,0.05)'))
    fig.update_layout(template='plotly_dark', height=300, margin=dict(l=0,r=0,t=10,b=0), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, color='#666'), yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)', color='#666'))
    ...
    trades_data = [
        {"Time": datetime.now().strftime('%H:%M'), "Pair": "BTCUSDT", "Type": "BUY", "PnL": "+$0.42"},
        {"Time": (datetime.now() - timedelta(hours=1)).strftime('%H:%M'), "Pair": "ETHUSDT", "Type": "SELL", "PnL": "+$0.15"},
        {"Time": (datetime.now() - timedelta(hours=2)).strftime('%H:%M'), "Pair": "SOLUSDT", "Type": "BUY", "PnL": "-$0.08"},
    ]
        ...
        if st.button(t['save_verify'], type="primary", width='stretch'):
            if not api_key or not api_secret: st.error(t['fill_fields'])
                ...
                if is_admin: client["api_key"], client["api_secret"], client["exchange"] = api_key, api_secret, exchange
                    ...
                    cl = load_clients(); cl["clients"][username]["api_key"] = api_key; cl["clients"][username]["api_secret"] = api_secret; cl["clients"][username]["exchange"] = exchange
    ...
    with col2:
        if client.get('api_key') and st.button(t['remove_keys'], width='stretch'):
            if is_admin: client["api_key"] = client["api_secret"] = client["exchange"] = ""
                ...
                cl = load_clients(); cl["clients"][username]["api_key"] = cl["clients"][username]["api_secret"] = cl["clients"][username]["exchange"] = ""
                save_clients(cl); st.session_state.client = cl["clients"][username]
    ...
    if is_admin:
        with tab_list[ti]:
            ...
            total = len(clist); active = sum(1 for c in clist if c.get("active")); api_ok = sum(1 for c in clist if c.get("api_key"))
            ...
            with a1:
                if clist:
                    data = []
                    for c in clist:
                        exp = datetime.fromisoformat(c["expires"]); dl = max(0, (exp - datetime.now()).days)
